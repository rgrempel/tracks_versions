...
require Rails.root.join('app', 'models', 'page')
...
class ActsAsVersionedTest < ActiveSupport::TestCase
  ...
  def test_rollback_with_version_number
    p = pages(:welcome)
    assert_equal days_ago_as_time(1), p.versions.last.version_from
    assert_equal 'Welcome to the weblog', p.title

    assert p.revert_to!(days_ago_as_time(2)), "Couldn't revert to 2 days ago"
  ...
  def test_rollback_with_version_class
    ...
    assert p.revert_to!(p.versions.earliest), "Couldn't revert to 23"
    assert_equal 'Welcome to the weblg', p.title
  end
  ...
  def test_rollback_with_version_number_with_options
    ...
    assert p.revert_to!(p.versions.earliest.version_from), "Couldn't revert to 23"
    assert_equal 'Welcome to the weblg', p.title
    assert_equal 'LockedPage', p.versions.first.version_type
  end
  ...
  def test_rollback_with_version_class_with_options
    ...
    assert p.revert_to!(p.versions.earliest), "Couldn't revert to 1"
    assert_equal 'Welcome to the weblg', p.title
    assert_equal 'LockedPage', p.versions.first.version_type
  end
  ...
  def test_version_no_limit
    ...
    5.times do |i|
      ...
      assert_equal (i+2), p.versions.count
    end
  end
  ...
  def test_find_version
    assert_equal page_versions(:welcome_1).attributes, pages(:welcome).versions.find_by_version_from(days_ago_as_time(2)).attributes
  end
  ...
  def test_referential_integrity
    # Note that I don't destroy versions automatically ... unlike AAV
  ...
  def test_association_options
    ...
    assert_equal nil, options[:dependent] # Note that I've changed the default from AAV

    association = Widget.reflect_on_association(:versions)
    options = association.options
    assert_equal nil, options[:dependent]
    assert_equal 'version_from desc', options[:order]
